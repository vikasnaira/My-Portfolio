  ...
  return (
    ...
    <div className={`bg-black-500/20 backdrop-blur-2xl flex md:hidden   flex-col h-screen fixed ${open? "translate-x-1000" :"translate-x-0 "} transition-transform duration-300 ease-in-out z-[998] flex  items-end p-6  w-full  text-white`}>
      <ul className='text-xl h-fit w-full py-20 px-10 vk flex flex-col gap-6 font-extrabold text-white'>
        <a href='#Hero' className='cursor-pointer hover:text-gray-400' onClick={() => handleMobileJump('Hero')}>HOME</a>
        <a href='#Projects' className='cursor-pointer hover:text-gray-400' onClick={() => handleMobileJump('Projects')}>WORK</a>
        <a href='#About' className='cursor-pointer hover:text-gray-400' onClick={() => handleMobileJump('About')}>ABOUT</a>
        <a href='#Contact2' className='cursor-pointer hover:text-gray-400' onClick={() => handleMobileJump('Contact')}>CONTACT</a>
      </ul>
    </div>
    </>
  )
